...
<link rel="stylesheet" href="https://code.jquery.com/ui/1.13.2/themes/base/jquery-ui.css">
  ...
  <body class="u-body u-xl-mode" data-lang="en"><header class="u-clearfix u-header" id="sec-fba2" data-animation-name="" data-animation-duration="0" data-animation-delay="0" data-animation-direction=""><div class="u-clearfix u-sheet u-sheet-1">
			...
			<tbody>
					<c:forEach var="bvo" items="${requestScope.ar }" varStatus="st">
						<tr>
							...
							<td><a href="javascript:dialogue('${bvo.q_idx }')" id="titleclick">${bvo.q_title }</a></td>
							<td>${bvo.q_writer }</td>
							<td>${bvo.q_hit }</td>
							<td>${bvo.q_date }</td>
						</tr>
			...
			<tfoot>
					<tr>
						...
						<ol class="paging">
							...
							<c:if test="${page.startPage >= page.pagePerBlock }">
								<li><a href="Controller?type=qna&cPage=${page.startPage-page.pagePerBlock }">&lt;</a></li>
							</c:if>
							<c:forEach begin="${page.startPage }" end="${page.endPage }" varStatus="st">
								...
								<c:if test="${page.nowPage ne st.index }">
									<li><a href="Controller?type=qna&cPage=${st.index }">${st.index }</a></li>
								</c:if>
							</c:forEach>	
	...
	<div id="search_popup" class="popup" title="글검색">
		<c:if test="${sessionScope.mvo != null }"><button type="button" id="write" onclick="javascript:location.href='Controller?type=qnawrite'">글쓰기</button></c:if>
		<c:if test="${sessionScope.mvo == null }"><button type="button" id="write" onclick="javascript:location.href='login.jsp'">글쓰기</button></c:if>
    ...
    <form action="Controller" method="post" name="frm">
		<input type="hidden" name="type" value="qnaview"/>
		<input type="hidden" name="q_idx" id="q_idx"/>
		<input type="hidden" name="q_pw" id="q_pw"/>
	</form>
  
	
	<div id="passblock" title="비밀번호 확인">
		<label for="pass">비밀번호:</label>
		<input type="password" id="pwd" name="pwd">
		<button type="button" id="ok_btn" onclick="view()">확인</button>
	</div>
	...
	function view(){
		...
		//$("#q_idx").val(qIdx);// 현재 문서안에 아이디가 q_idx인 요소의 value로 인자로 받은 qIdx의 값으로 지정한다.
