...
<link rel="stylesheet" href="product.css" media="screen">
  ...
  <body class="u-body u-xl-mode" data-lang="en"><header class="u-clearfix u-header u-palette-5-light-2 u-sticky" id="sec-0db4" data-animation-name="" data-animation-duration="0" data-animation-delay="0" data-animation-direction=""><div class="u-clearfix u-sheet u-sheet-1">
    ...
    <section class="u-align-center u-clearfix u-section-1" id="sec-9284">
        ...
        <div class="u-form u-form-1">
          <form action="Controller_admin" class="u-clearfix u-form-horizontal u-form-spacing-15 u-inner-form" style="padding: 15px" source="email">
            <div class="u-form-group u-form-select u-label-none u-form-group-1">
              ...
              <div class="u-form-select-wrapper">
                <select id="select-8570" name="searchType" class="u-input u-input-rectangle u-radius-50">
                  <option value="0" data-calc="p_idx">상품번호</option>
                  <option value="1" data-calc="p_name">상품명</option>
                  <option value="2" data-calc="p_artist">작가명</option>
            ...
            <div class="u-form-group u-form-submit u-label-none">
              ...
              <button onclick="submitData(this.form)" type="button" class="u-border-none u-btn u-btn-round u-btn-submit u-button-style u-grey-50 u-hover-grey-60 u-radius-50 u-btn-1">검색</button>
            </div>
            ...
            <!-- <input type="hidden" name="type" value="admin_search"> -->
            <input type="hidden" name="type" value="admin_product">
          </form>
        </div>
        <div class="u-form u-form-2">
          <form action="Controller_admin" class="u-clearfix u-form-horizontal u-form-spacing-15 u-inner-form" style="padding: 15px" source="email">
            <div class="u-form-group u-form-select u-label-none u-form-group-4">
              <label for="select-8570" class="u-form-control-hidden u-label u-label-3"></label>
              <div class="u-form-select-wrapper">
                <select id="select-8570" name="select" class="u-input u-input-rectangle u-radius-50">
                  <option value="0" <c:if test="${requestScope.select == 0 }">selected</c:if>>판매신청</option>
                  <option value="1" <c:if test="${requestScope.select == 1 }">selected</c:if>>판매대기</option>
                  <option value="2" <c:if test="${requestScope.select == 2 }">selected</c:if>>판매중</option>
                  <option value="3" <c:if test="${requestScope.select == 3 }">selected</c:if>>판매완료</option>
                  <option value="4" <c:if test="${requestScope.select == 4 }">selected</c:if>>판매거절</option>
                  <option value="5" <c:if test="${requestScope.select == 5 }">selected</c:if>>판매취소</option>
                </select>
                <svg class="u-caret u-caret-svg" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="16px" height="16px" viewBox="0 0 16 16" style="fill:currentColor;" xml:space="preserve"><polygon class="st0" points="8,12 2,4 14,4 "></polygon></svg>
              </div>
            </div>
            <div class="u-form-group u-form-submit u-label-none">
              <input type="submit" value="submit" class="u-form-control-hidden">
              <button onclick="submitData(this.form)" type="button" class="u-border-none u-btn u-btn-round u-btn-submit u-button-style u-grey-50 u-hover-grey-60 u-radius-50 u-btn-2">정렬</button>
            </div>
            <div class="u-form-send-message u-form-send-success"></div>
            <div class="u-form-send-error u-form-send-message"></div>
            <input type="hidden" value="" name="recaptchaResponse">
    ...
    <section class="u-clearfix u-section-2" id="sec-f8e2">
      <div class="u-clearfix u-sheet u-sheet-1">
          ...
          <table class="u-table-entity">
            ...
            <thead class="u-grey-50 u-table-header u-table-header-1">
              <tr style="height: 45px;">
                <th class="u-border-1 u-border-grey-50 u-table-cell">상품번호</th>
                <th class="u-border-1 u-border-grey-50 u-table-cell">이미지</th>
                <th class="u-border-1 u-border-grey-50 u-table-cell">상품명</th>
                <th class="u-border-1 u-border-grey-50 u-table-cell">작가명</th>
                <th class="u-border-1 u-border-grey-50 u-table-cell">가격</th>
                <th class="u-border-1 u-border-grey-50 u-table-cell">판매상태</th>
              </tr>
            </thead>
            ...
            <c:forEach var="pvo" items="${requestScope.pvo }">
              <tr style="height: 180px;">
                <td class="u-border-1 u-border-grey-40 u-border-no-left u-border-no-right u-table-cell">${pvo.p_idx }</td>
                <td class="u-border-1 u-border-grey-40 u-border-no-left u-border-no-right u-table-cell"><span class="u-file-icon u-icon u-icon-1"><a href="Controller_admin?type=product_detail&product=${pvo.p_idx }"><img src="image_upload/${pvo.file_name }" style="width:150px;"></a></span><span class="u-icon"></span>
                </td>
                <td class="u-border-1 u-border-grey-40 u-border-no-left u-border-no-right u-table-cell"><a href="Controller?type=product_detail&product=${pvo.p_idx }">${pvo.p_name }</a></td>
                <td class="u-border-1 u-border-grey-40 u-border-no-left u-border-no-right u-table-cell">${pvo.m_id }</td>
                <td class="u-border-1 u-border-grey-40 u-border-no-left u-border-no-right u-table-cell">
					<c:if test="${not empty pvo.p_s_price and pvo.p_status == 0 or pvo.p_status == 1 or pvo.p_status == 4 or pvo.p_status == 5 or pvo.avo.a_price == pvo.p_s_price}">
	 					<fmt:formatNumber pattern="#,###" value="${pvo.p_s_price}" />원
 					</c:if>
 					<c:if test="${not empty pvo.avo.a_price and pvo.avo.a_status != 0}">
  				 		<fmt:formatNumber pattern="#,###" value="${pvo.avo.a_price}" />원
 					</c:if>
                ...
                <c:if test="${pvo.p_status eq '1'}">
                	판매 대기
                </c:if>
                <c:if test="${pvo.p_status eq '2'}">
                	판매 중
                </c:if>
                <c:if test="${pvo.p_status eq '3'}">
                	판매 완료
                </c:if>
                <c:if test="${pvo.p_status eq '4'}">
                	판매 거절
                </c:if>
              	<c:if test="${pvo.p_status eq '5'}">
            			판매 취소
             		</c:if>
                </td>
              </tr>
            ...
            <tfoot>
					<tr>
						...
						<ol class="paging">
							...
							<c:if test="${page.startPage >= page.pagePerBlock }">
								<c:if test="${requestScope.searchType !=null and requestScope.searchValue != null}">
									<li><a href="Controller?type=admin_product&cPage=${page.startPage-page.pagePerBlock }&searchType=${requestScope.searchType}&searchValue=${requestScope.searchValue}">&lt;</a></li>
								</c:if>
								<c:if test="${requestScope.searchType eq null or requestScope.searchValue eq null}">
									<li><a href="Controller?type=admin_product&cPage=${page.startPage-page.pagePerBlock }">&lt;</a></li>
								</c:if>
								<c:if test="${requestScope.searchType eq null and requestScope.select ne null}">									
									<li><a href="Controller?type=admin_product&cPage=${page.startPage-page.pagePerBlock }&select=${requestScope.select}">&lt;</a></li>
								</c:if>
								<c:if test="${requestScope.searchType ne null and requestScope.select ne null}">									
									<li><a href="Controller?type=admin_product&cPage=${page.startPage-page.pagePerBlock }&searchType=${requestScope.searchType}&searchValue=${requestScope.searchValue}&select=${requestScope.select}">&lt;</a></li>
								</c:if>
							</c:if>
							...
							<c:forEach begin="${page.startPage }" end="${page.endPage }" varStatus="st">
								...
								<c:if test="${page.nowPage ne st.index }">
									<c:if test="${requestScope.searchType ne null and requestScope.select ne null}">
										<li><a href="Controller?type=admin_product&cPage=${st.index }&searchType=${requestScope.searchType}&searchValue=${requestScope.searchValue}&select=${requestScope.select}">${st.index }</a></li>
									</c:if>
									<c:if test="${requestScope.searchType ne null and requestScope.select eq null}">
										<li><a href="Controller?type=admin_product&cPage=${st.index }&searchType=${requestScope.searchType}&searchValue=${requestScope.searchValue}">${st.index }</a></li>
									</c:if>
									<c:if test="${requestScope.searchType eq null and requestScope.select eq null}">									
										<li><a href="Controller?type=admin_product&cPage=${st.index }">${st.index }</a></li>
									</c:if>
									<c:if test="${requestScope.searchType eq null and requestScope.select ne null}">									
										<li><a href="Controller?type=admin_product&cPage=${st.index }&select=${requestScope.select}">${st.index }</a></li>
									</c:if>
								</c:if>
							</c:forEach>	
    ...
    <input type="hidden" id="chk_select" value="${requestScope.select }"/>
    <input type="hidden" id="chk_type" value="${requestScope.searchType }"/>
    <input type="hidden" id="chk_value" value="${requestScope.searchValue }"/>
