  ...
  <body class="u-body u-xl-mode" data-lang="en"><header class="u-clearfix u-header" id="sec-fba2" data-animation-name="" data-animation-duration="0" data-animation-delay="0" data-animation-direction=""><div class="u-clearfix u-sheet u-sheet-1">
    ...
    <section class="u-clearfix u-section-1" id="sec-f563">
      <div class="u-clearfix u-sheet u-sheet-1">
       ...
       <div id="bbs">
		<form action="Controller?type=qnacommenteditcomplete" method="post">
		<table summary="QnA 수정">
			<caption class="caption">QnA 댓글 수정</caption>
			<c:set var="bvo3" value="${requestScope.vo4 }"></c:set>
			<tbody>	
				<tr>
					<td><textarea name="content" cols="80" rows="10" id="content" data-str="내용"}>${bvo3.c_content }</textarea></td>
				</tr>
			</tbody>
		</table>
		<button type="button" id="write" onclick="sendData()">저장</button>
		<button type="button" id="write" onclick="javascript:location.href='Controller?type=qnaview&q_idx=${bvo3.q_idx }'">취소</button>
		<input type="hidden" name="c_idx" value="${bvo3.c_idx }"/>
		<input type="hidden" name="m_idx" value="${bvo3.m_idx }"/>
		<input type="hidden" name="q_idx" value="${bvo3.q_idx }"/>
		<input type="hidden" name="q_pw" value="${requestScope.vo3.q_pw }"/>
		</form>
	</div>
